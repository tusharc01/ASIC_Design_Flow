...
- **Routing**: Created physical metal wire connections between placed cell pins, following the netlist logic while adhering to design rules (DRCs) from LEF and technology files. Encompasses global routing (planning general paths), track assignment (assigning nets to tracks), and detailed routing (creating exact wire segments and vias).  
  ...
  - **Track Assignment Details**: This sub-step (mentioned in Tool Action) assigns nets to predefined routing tracks (horizontal/vertical channels in the layout grid) after global routing but before detailed routing. It's chosen to organize wires efficiently, reducing overlaps and enabling easier via insertion; parameters like track width/spacing are set per layer to match technology rules, ensuring minimal detours and better signal integrity—e.g., denser tracks in lower layers for local connections in this design to save upper layers for clocks/power.  
  - **How to Choose Parameters (Designer Knowledge)**: Selections balance design goals (e.g., timing, area, power) with technology constraints (e.g., 180nm rules on min-width/spacing); iterated via DRC reports and congestion maps to achieve clean routing with `.
